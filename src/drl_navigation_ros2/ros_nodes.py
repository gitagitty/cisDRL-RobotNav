...
class SensorSubscriber(Node):
    def __init__(self):
        ...
        self.touch_sub = self.create_subscription(
            ContactsState, "robot/cfg/touched", self.touch_listener_callback, 10
        )
    ...
    def touch_listener_callback(self, msg):
        ...
        for contact in msg.states:
            ...
            if hasattr(contact, 'collision1_name') and hasattr(contact, 'collision2_name'):
                ...
                # 过滤掉地面平面碰撞
                if 'wheel' in collision1 and 'ground' in collision2 :
    ...
    def reset_collision_count(self):
        self.collision_count = 0
        # self.get_logger().info("Collision counter reset to zero")
